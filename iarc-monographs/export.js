...
let iarc = { mod_nlp: null }

/**
 * Main global portable module.
 * @namespace 
 * @property {Function} IarcGpt - {@link IarcGpt}
 *
 * @namespace iarc
 * @property {Function} IarcGpt - {@link iarc.IarcGpt}
 * @property {Function} getValidatedData - {@link iarc.getValidatedData}
 * @property {Function} init_nlp - {@link iarc.init_nlp}
 * @property {Function} get_section_slice - {@link iarc.get_section_slice}
 * @property {Function} get_header_sections - {@link iarc.get_header_sections}
 * @property {Function} loadMonograph - {@link iarc.loadMonograph}
 * @property {Function} remove_breaks - {@link iarc.remove_breaks}
 * @property {Function} remove_number_short_sentences - {@link iarc.remove_number_short_sentences}
 * @property {Function} calculate_sim - {@link iarc.calculate_sim}
 * @property {Function} filter_context_sections - {@link iarc.filter_context_sections}
 * @property {Function} filter_context_sentences - {@link iarc.filter_context_sentences}
 ...
 * @property {Function} get_gpt_answers - {@link iarc.get_gpt_answers}
 * @property {Function} _prepTask - {@link iarc._prepTask}
 * @property {Function} get_wink_answers - {@link iarc.get_wink_answers}
...
* let v = await IarcGpt()
*/
var IarcGpt = async function (key=''){
    localStorage.GPT_API_key = key
    
    var obj = {'monographs': []}
    var info = await Promise.all( [ import('https://episphere.github.io/gpt/jonas/export.js'), iarc.loadScrapedMonographs(), iarc.getValidatedData(obj), iarc.init_nlp() ] )
    obj.gpt = info[0]
...
* 
*
* @param {Object} iarcobj IarcGpt library object
*
...
* 
* @example
* let obj = await IarcGpt()
* let dat = await iarc.getValidatedData(obj)
*/
...
iarc.init_nlp = async () => {
    ...
    if(iarc.mod_nlp==null){
        iarc.mod_nlp={}
        iarc.mod_nlp['winkNLP'] = (await import('https://cdn.skypack.dev/wink-nlp')).default;
        iarc.mod_nlp['model'] = (await import('https://cdn.skypack.dev/wink-eng-lite-web-model')).default;
        iarc.mod_nlp['bm25Vectorizer'] = (await import('https://cdn.skypack.dev/wink-nlp/utilities/bm25-vectorizer')).default
        ...
        // Bert tfjs
        iarc.mod_nlp['bertModel'] = await qna.load()
    }
    
    console.log('iarc nlp module loaded')
...
* 
* @example
* let v = await iarc.loadMonograph('https://publications.iarc.fr/_publications/media/download/5409/5f12a3cfc6a2291b8dca418b2d322642ccb8f0fc.pdf')
*/
...
* let v = await iarc.get_header_sections(content)
*/
iarc.get_header_sections = (content) => {
    var info_sections = []
            ...
            if(temp!=undefined){
                ...
                for(var el of els ){
                    if(el.indexOf('. . . . . .')!=-1){
                        ...
                        if(previous!=-1 && flag){
                            info_sections[ info_sections.length-1 ].content = iarc.get_section_slice(content, previous+add, page+add)
...
* @example
* let content = await iarc.loadMonograph('https://publications.iarc.fr/_publications/media/download/5409/5f12a3cfc6a2291b8dca418b2d322642ccb8f0fc.pdf')
* let v = await iarc.get_section_slice(content, 10, 15)
*/
iarc.get_section_slice = (content, start, end) => {
    var text=''
    for(var i=start; i<end+1; i++){
        var temp = content.filter( c => c.page==i )[0]
        text+=temp.content+"\n"
    }
    text = text.split('.').filter(e => e.length >= 30 ).map(e => iarc.remove_breaks(e))
    return text
}
...
* 
* @example
* let v = await iarc.calculate_sim('tversky', 'Is there evidence for the carcinogenicity of opium consumption?', 'Opium consumption causes cancers of the urinary bladder, larynx, and lung')
*/
iarc.calculate_sim = (type, question, sentence) => {
    var sim_types = ['cosine', 'tversky', 'oo']
    type = ( sim_types.includes(type) ) ? type : 'cosine'
    
    if( sim_types.includes(type) ){
        var docq = iarc.mod_nlp.nlp.readDoc(question);
        var docs = iarc.mod_nlp.nlp.readDoc(sentence);
    
        var fam = (type=='cosine') ? 'bow' : 'set'
        var featuresq = docq.tokens().out( iarc.mod_nlp.its.value, iarc.mod_nlp.nlp.as[fam] )
        var featuress = docs.tokens().out( iarc.mod_nlp.its.value, iarc.mod_nlp.nlp.as[fam] )
        
        //return { 'sentence': sentence, 'score': iarc.mod_nlp.sim[fam][type](featuresq, featuress) }
        return  iarc.mod_nlp.sim[fam][type](featuresq, featuress)
    }
    else{
...
* @example
* let content = await iarc.loadMonograph('https://publications.iarc.fr/_publications/media/download/5409/5f12a3cfc6a2291b8dca418b2d322642ccb8f0fc.pdf')
* let sections = await iarc.get_header_sections(content)
* let v = await iarc.filter_context_sections('Is there evidence for the carcinogenicity of opium consumption?', sections, 'tversky')
*/
iarc.filter_context_sections = (q, sections, st) => {
    var sim_types = ['cosine', 'tversky', 'oo']
    ...
    for (var section of sections){
        ...
        sentences = iarc.remove_number_short_sentences(sentences, 3)
        sentences = sentences.map( s => { return s.toLowerCase() } )
        q = q.toLowerCase()
        var x = sentences.map( s => { return { 'section': k, 'sentence': s, 'similarity': iarc.calculate_sim(st, q, s) } } )
        all = all.concat(x)
...
* 
* @example
* let v = await iarc.filter_context_sentences('Is there evidence for the carcinogenicity of opium consumption?', ['1 Cancer in humans\n There is sufficient evidence in humans for the carcinogenicity of opium consumption', '2 Cancer in experimental animals\n There is inadequate evidence in experimental animals regarding the carcinogenicity of opium'], 'tversky')
*/
iarc.filter_context_sentences = (q, sentences, st) => {
    var sim_types = ['cosine', 'tversky', 'oo']
    st = ( sim_types.includes(st) ) ? st : 'cosine'
    ...
    var metrics = {'overall': [] }
    q = q.toLowerCase()
    var x = sentences.map( s => { return { 'sentence': s, 'similarity': iarc.calculate_sim(st, q, s) } } )
    all = all.concat(x)
    metrics['overall'] = {}
    var scores = all.map(e => e['similarity'])
    for (f of ['max','min','mean']){
...
* 
* @example
* let v = await iarc.correct_sentence_grammar(" In reaching this determina tion, the Wking Grup noted that in a cohort study of 50 045 adults in Golestan Province, a north-eatern province of the Islamic Republic of Iran, self-reported opium consumption was assessed at baseline, validated with urinary levels of opium metabolites, and the cohort was followed for more than a decade to ascertain inci dent cancers", 'gpt-3.5-turbo', 0.7)
*/
iarc.correct_sentence_grammar = async function (sentence, model='gpt-3.5-turbo', temperature=0.7){
...
* 
* @example
* let v = await iarc.correct_agents_from_gpt("There is sufficient evidence in humans for the carcinogenicity of opium consumption. Opium consumption causes cancers of the urinary bladder, larynx, and lung. Positive associations have been observed between opium consumption and cancers of the oesophagus, stomach, pancreas, and pharynx.2 Cancer in experimental animals There is inadequate evidence in experimental animals regarding the carcinogenicity of opium.3 Mechanistic evidence There is strong evidence in experimental systems that opium, specifically sukhteh and opium pyrolysates, exhibits key characteristics of carcinogens (it is genotoxic).4 Overall evaluation Opium consumption is carcinogenic to humans (Group 1) ", 'gpt-3.5-turbo', 0.7)
*/
iarc.correct_agents_from_gpt = async function ( eval_paragraph, model='gpt-3.5-turbo', temperature=0.7){
    ...
    if(key!='' && key!=null && key!="none"){
        var res = await 
         ...
         var ans = iarc.process_agent_gpt_result(res)
         return ans
    }
    else{
        console.log('Error: Openai key was not found.')
...
* 
*/
iarc.process_agent_gpt_result = (obj) => {
    var agents=[]
    if( obj.choices != null ){
        if(obj.choices.length > 0 ){
...
* 
* @example
* let v = await iarc.get_bert_answers( "Is there evidence for the carcinogenicity of opium consumption?", ["There is sufficient evidence in humans for the carcinogenicity of opium consumption.", "Opium consumption causes cancers of the urinary bladder, larynx, and lung.", "Positive associations have been observed between opium consumption and cancers of the oesophagus, stomach, pancreas, and pharynx.", "2 Cancer in experimental animals There is inadequate evidence in experimental animals regarding the carcinogenicity of opium.", "3 Mechanistic evidence There is strong evidence in experimental systems that opium, specifically sukhteh and opium pyrolysates, exhibits key characteristics of carcinogens (it is genotoxic).", "4 Overall evaluation Opium consumption is carcinogenic to humans (Group 1)"] )
*/
iarc.get_bert_answers = async (q, context) => {
    var corpus = context.join('. ')
    var answers = await iarc.mod_nlp.bertModel.findAnswers(q, corpus)
    return answers
}
...
* let v = await iarc.completions(msgs, 'gpt-3.5-turbo', 0.7)
*/
...
* 
* @example
* let v = await iarc.get_gpt_answers( "Is there evidence for the carcinogenicity of opium consumption?", ["There is sufficient evidence in humans for the carcinogenicity of opium consumption.", "Opium consumption causes cancers of the urinary bladder, larynx, and lung.", "Positive associations have been observed between opium consumption and cancers of the oesophagus, stomach, pancreas, and pharynx.", "2 Cancer in experimental animals There is inadequate evidence in experimental animals regarding the carcinogenicity of opium.", "3 Mechanistic evidence There is strong evidence in experimental systems that opium, specifically sukhteh and opium pyrolysates, exhibits key characteristics of carcinogens (it is genotoxic).", "4 Overall evaluation Opium consumption is carcinogenic to humans (Group 1)"] )
*/
iarc.get_gpt_answers = async (q, context) => {
    var corpus = context.join('. ')
    ...
    var answer = await iarc.completions( obj['messages'], 'gpt-3.5-turbo', 0.7)
    obj['answer'] = answer
...
* 
* @example
* let v = await iarc._prepTask( "There is sufficient evidence in humans for the carcinogenicity of opium consumption. Opium consumption causes cancers of the urinary bladder, larynx, and lung. Positive associations have been observed between opium consumption and cancers of the oesophagus, stomach, pancreas, and pharynx. 2 Cancer in experimental animals There is inadequate evidence in experimental animals regarding the carcinogenicity of opium. 3 Mechanistic evidence There is strong evidence in experimental systems that opium, specifically sukhteh and opium pyrolysates, exhibits key characteristics of carcinogens (it is genotoxic).4 Overall evaluation Opium consumption is carcinogenic to humans (Group 1)" )
*/
iarc._prepTask = function ( text ) {
  const tokens = [];
  iarc.mod_nlp.nlp.readDoc(text)
      .tokens()
      // Use only words ignoring punctuations etc and from them remove stop words
      .filter( (t) => ( t.out( iarc.mod_nlp.its.type) === 'word' && !t.out( iarc.mod_nlp.its.stopWordFlag) ) )
      // Handle negation and extract stem of the word
      .each( (t) => tokens.push( (t.out( iarc.mod_nlp.its.negationFlag)) ? '!' + t.out( iarc.mod_nlp.its.stem) : t.out( iarc.mod_nlp.its.stem) ) );
...
* 
* @example
* let v = await iarc.get_wink_answers( "Is there evidence for the carcinogenicity of opium consumption?", ["There is sufficient evidence in humans for the carcinogenicity of opium consumption.", "Opium consumption causes cancers of the urinary bladder, larynx, and lung.", "Positive associations have been observed between opium consumption and cancers of the oesophagus, stomach, pancreas, and pharynx.", "2 Cancer in experimental animals There is inadequate evidence in experimental animals regarding the carcinogenicity of opium.", "3 Mechanistic evidence There is strong evidence in experimental systems that opium, specifically sukhteh and opium pyrolysates, exhibits key characteristics of carcinogens (it is genotoxic).", "4 Overall evaluation Opium consumption is carcinogenic to humans (Group 1)"] )
*/
iarc.get_wink_answers = async (q, context) => {
    var corpus = iarc.remove_number_short_sentences(context)
    corpus = corpus.map( s => { return { 'body': s} } )
    
    var query = q;

    var engine = iarc.mod_nlp.bm25();
    // Step I: Define config
    // Only field weights are required in this example.
    engine.defineConfig( { fldWeights: { body: 2 } } );
    // Step II: Define PrepTasks pipe.
    // Set up 'default' preparatory tasks i.e. for everything else
    engine.definePrepTasks( [ iarc._prepTask ] );
...
* 
* @example
* let v = await iarc.loadScrapedMonographs()
*/
...
* 
* @example
* let v = await iarc.getExtractLines('https://publications.iarc.fr/Book-And-Report-Series/Iarc-Monographs-On-The-Identification-Of-Carcinogenic-Hazards-To-Humans/1-1-1-Trichloroethane-And-Four-Other-Industrial-Chemicals-2022')
*/
...
* 
* @example
* let lines = await iarc.processMonographLinkHtml('https://publications.iarc.fr/Book-And-Report-Series/Iarc-Monographs-On-The-Identification-Of-Carcinogenic-Hazards-To-Humans/1-1-1-Trichloroethane-And-Four-Other-Industrial-Chemicals-2022')
* var links = {}
* links = await iarc.processMonographLinkHtml(links, lines)
*/
...
* var links = {}
* links = await iarc.processMonographLinkHtml(links, lines)
* var dat = await iarc.getBookLinks(links)
*/
...
* 
* @example
* var dat = await iarc.scrapSourceMonoGraphLinks()
*/
